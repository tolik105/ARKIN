...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 w-full z-50 backdrop-blur-md bg-white/10 dark:bg-neutral-900/30 ring-1 ring-white/10 dark:ring-white/5">
    ...
    >
      ...
      <motion.div className="lg:flex flex-row flex-1 absolute inset-0 hidden items-center justify-center space-x-2 lg:space-x-2 text-sm text-zinc-600 font-medium hover:text-zinc-800 transition duration-200">
        {navItems.map((navItem: any, idx: number) => (
          ...
          >
            ...
            <span className="relative z-20 px-4">{navItem.name}</span>
          </Link>
        ))}
      </motion.div>
      <div className="flex items-center gap-4">
        ...
        <Button
          as={Link}
          href="#waitlist"
          className="hidden md:flex space-x-2 items-center bg-gradient-to-b from-blue-500 to-blue-700 text-white px-4 py-2 rounded-lg shadow-lg hover:from-blue-600 hover:to-blue-800 transition-all duration-200 font-medium"
        >
          <span>Join Waitlist</span>
          <HiArrowRight className="group-hover:translate-x-1 stroke-[1px] h-3 w-3 mt-0.5 transition-transform duration-200" />
        </Button>
